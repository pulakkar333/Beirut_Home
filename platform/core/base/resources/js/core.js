...
class Botble {
    ...
    static initCoreIcon() {
        ...
        const formatTemplate = ({ id, text }) => {
            ...
            return $(
                `<span><span class="dropdown-item-indicator">${text}</span> ${id}</span>`,
            );
        };
        ...
        Botble.noticesTimeout[key] = setTimeout(() => {
            if (!messageHeader) {
                switch (messageType) {
                    case "error":
                        messageHeader =
                            BotbleVariables.languages.notices_msg.error;
                        break;
                    case "success":
                        messageHeader =
                            BotbleVariables.languages.notices_msg.success;
                        break;
                }
            }
    ...
    countCharacter() {
        $.fn.charCounter = function (max, settings) {
            ...
            let count = (el, container) => {
                ...
                container.html(
                    settings.format.replace(/%1/, current).replace(/%2/, max),
                );
        ...
        $(document).on(
            ...
            (event) => {
                ...
                $(event.currentTarget).charCounter($this.data("counter"), {
                    ...
                    allowOverLimit:
                        $this.data("allow-over-limit") == "" ? true : false,
                });
            },
        );
    }
                ...
                if (navbar.length > 0) {
                    ...
                    $httpClient
                        ...
                        .post(_self.data("url"), {
                            ...
                            minimal_sidebar: navbar.hasClass("navbar-minimal")
                                ? "yes"
                                : "no",
                        })
                        .then(() => {})
    ...
    static initResources() {
        ...
        $(document)
            ...
            .each(function (index, element) {
                const $checkboxChildren = $(document).find(
                ...
                if (
                    $checkboxChildren.length ===
                    $checkboxChildren.filter(":checked").length
                ) {
    ...
    handleTurnOffDebugMode() {
        const debugConfirmationModal = $(document).find(
        ...
        const $submit = debugConfirmationModal.find(
            "#debug-mode-turn-off-form-submit",
        );
    ...
    static initMediaIntegrate() {
        if (jQuery().rvMedia) {
                        ...
                        onSelectFiles: (files, $el) => {
                            switch ($el.data("action")) {
                                ...
                                case "attachment":
                                    ...
                                    break;
                                default:
                                    const coreInsertMediaEvent =
                                        new CustomEvent("core-insert-media", {
                                            detail: {
                                                files: files,
                                                element: $el,
                                            },
            ...
            const gallerySelectImages = function (
                files,
                $currentBoxList,
                excludeIndexes = [],
            ) {
                ...
                _.forEach(files, (file, index) => {
                    ...
                    if (!allowThumb) {
                        $template
                            .find(".preview-image-wrapper")
                            .addClass("preview-image-wrapper-not-allow-thumb");
                    }
            ...
            new RvMediaStandAlone('[data-bb-toggle="image-picker-edit"]', {
                ...
                onSelectFiles: (files, $el) => {
                    ...
                    $currentBox
                        ...
                        .attr(
                            "src",
                            allowThumb ? firstItem.thumb : firstItem.full_url,
                        )
                        .show();
                ...
                function (index, item) {
                    ...
                    _self.rvMedia({
                        ...
                        onSelectFiles: (files, $el) => {
                            ...
                            $imageBox
                                .find('[data-bb-toggle="image-picker-remove"]')
                                .show();
                            $imageBox
                                .find(".preview-image")
                                .removeClass("default-image");
                            ...
                            const coreInsertMediaEvent = new CustomEvent(
                                "core-insert-media",
                                {
                                    detail: {
                                        files: files,
            ...
            if (form.find("#download_image_to_local_storage").is(":checked")) {
                $httpClient
                    ...
                    .then(({ data }) => {
                        ...
                        const $imageBox = $(
                            form.find('input[name="image-box-target"]').val(),
                        );
                        $imageBox
                            .find(".image-data")
                            .val(data.data.url)
        ...
        $(document).on(
            ...
            (e) => {
                ...
                if ($list.find(".gallery-image-item-handler").length === 0) {
                    ...
                    $listImage
                        .find(".default-placeholder-gallery-image")
                        .removeClass("hidden");
    ...
    static initCodeEditor(id, type = "css") {
        ...
        editor.wrap(
            `<div id='wrapper_${id}'><div class='container_content_codemirror'></div> </div>`,
        );
        ...
        $(".handle-tool-drag").mousedown((event) => {
            ...
            $("body")
                ...
                .on("mousemove", Botble.onDragTool);
    ...
    static onDragTool(e) {
        ...
        $element
            ...
            .css(
                ...
                Math.max(
                    200,
                    startHeight + e.pageY - $element.attr("data-start_y"),
                ),
            );
    }
    ...
    static initTreeCheckboxes() {
        ...
        const handleParents = function (current) {
            ...
            if ($topParent.find("ul input[type=checkbox]:checked").length > 0) {
                if (
                    $topParent.find("ul input[type=checkbox]:checked")
                        .length ===
    ...
    static select(element, options = {}, force = false) {
        ...
        if (
            !jQuery().select2 ||
            ($element.hasClass("select2-hidden-accessible") && !force)
        ) {
            return;
        }
    ...
    static initLightbox() {
        ...
        a.forEach((element) => {
            ...
            const currentIndex =
                instance[instanceName].props.sources.length - 1;
    ...
    static initClipboard() {
        $(document).on("click", '[data-bb-toggle="clipboard"]', async (e) => {
            ...
            const iconClipboardSuccess = target.find(
                "[data-clipboard-success-icon]",
            );
            const clipboardParent = target.data("clipboard-parent");
            const clipboardParentTarget = clipboardParent
                ? document.querySelector(clipboardParent)
                : undefined;
    ...
    static initDropdownCheckboxes() {
        const countCheckedDropdownCheckboxes = (e) => {
            ...
            const $checkedCheckboxes = $wrapper.find(
            ...
            if ($checkedCheckboxes.length) {
                if ($checkedCheckboxes.length > 3) {
                    ...
                    $checkedCheckboxes.each(function () {
                        values.push(
                            $(this).siblings(".form-check-label").text().trim(),
                        );
                    });
        ...
        $(document).on(
            ...
            (e) => {
                ...
                if ($(e.currentTarget).is(":checked")) {
                    ...
                    const $input = $selected.find(
                        `input[value="${$(e.currentTarget).val()}"]`,
                    );
                    $input.remove();
                }
            },
        ...
        $(document).on("click", function (event) {
            ...
            if (
                !$target.closest('[data-bb-toggle="dropdown-checkboxes"]')
                    .length
            ) {
